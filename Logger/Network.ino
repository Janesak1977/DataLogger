...
extern byte RespRcvd;
//extern byte boxRebootCount;
extern boolean plugTestRequest;
extern unsigned long OnTimeSecodns; 
extern word SentCount;
...
void NetworkInitialize()
{
  ...
  Serial.print("Initializing ENC28J60... ");
  // Init ENC28J60, must be done after SPI has been properly set up! (means RF12Init must be called before)
  if (eth.begin(sizeof Ethernet::buffer, mymac) == 0) 
    Serial.println( "Failed");
    else
    Serial.println("OK");

  // Initialize TCP stuff
  eth.staticSetup((byte *)Config.LocalIP, (byte *)Config.RouterIP);
  eth.printIp("Local IP:  ", eth.myip);
  eth.printIp("Gateway IP:  ", eth.gwip);  
...
//    for (byte i = 0; i < 4; i++) websrvip[i] = Config.ServerIP[i];
      ...
      websrvip[2] = 190;
      websrvip[3] = 58;
      ether.copyIp(ether.hisip, websrvip);
/*
    Serial.print("DNS lookup for website... "); 
    if (!ether.dnsLookup(website))
      Serial.println("Failed");
    else
    {
      Serial.print("OK - ");*/
      eth.printIp("IP: ", ether.hisip);
//    }
  //}
}
...
static void StatusPage(BufferFiller& buf)
{
  ...
  // Print uptime HH:MM:SS since last reset (Power up)
  word h = OnTimeSeconds / 3600;
  byte m = (OnTimeSeconds / 60) % 60;
  byte s = OnTimeSeconds % 60;
  buf.emit_p(PSTR("$D$D:$D$D:$D$D"), h/10, h%10, m/10, m%10, s/10, s%10);
  ...
  buf.emit_p(PSTR("<H3>IT+ Sensors</H3>"));
    ...
    if (ITPlusChannels[Channel].SensorID != 0xff)
    {
      if (ITPlusChannels[Channel].LastReceiveTimer != 0)
      {
        if ((ITPlusChannels[Channel].Temp & 0x80) != 0)
          buf.emit_raw("-", 1);
        buf.emit_p(PSTR("$D.$D&degC"), ITPlusChannels[Channel].Temp & 0x7f, ITPlusChannels[Channel].DeciTemp);
        if (ITPlusChannels[Channel].Hygro!=106)
          buf.emit_p(PSTR(" $D%"), ITPlusChannels[Channel].Hygro);
        if (ITPlusChannels[Channel].LoBat==0x80)
          buf.emit_p(PSTR(" LoBatt"));
      }
      else
      {
  ...
  // Print RF12 sensors
  buf.emit_p(PSTR("<H3>RF12 Sensors</H3>"));
  for (byte Channel = 0; Channel < MAX_JEENODE; Channel++)
  {
    if (Channel != 0) buf.emit_p(PSTR("<br/>"));
    buf.emit_p(PSTR("Ch$D: "), Channel+JEEDNODE_ENERGY_MON_NODEID_START);
    if (RF12Channels[Channel].Enabled)
    {
      if (RF12Channels[Channel].LastReceiveTimer != 0)
      {
        buf.emit_p(PSTR("$DW $DWh"), RF12Channels[Channel].Power, RF12Channels[Channel].Energy);
      }
      else
      {
        buf.emit_p(PSTR("Stalled"));
...
void browserresult_callback(uint8_t statuscode, uint16_t datapos, uint16_t len)
{
  ...
  switch (statuscode)
  {
    case 0:
      ...
      SentCount++;
      ethernet_requests = 0;
      RF12Channels[0].Energy = 0;
      break;

    case 1:
#if DEBUG_HTTP
      Serial.println("Resp:Err");
