...
//  PARAM_RESET_DATA_PORT |= (1 << PARAM_RESET_PIN_NB);
  ...
  if (CKS != eeprom_read_byte(&CONFIG_CKS_EEPROM)) {
    DebugPrintln_P(PSTR("EE_CKS Err"));

    // Copy initial values to config struct
    ResetConfig();
  }
  else
  {  // Checksum OK, Load the valid config if not reset requested
    DebugPrintln_P(PSTR("EE_CKS OK"));
 /*   
    ...
    if ((PARAM_RESET_PIN_PORT & (1 << PARAM_RESET_PIN_NB)) == 0)
    {
      ...
      DebugPrintln_P(PSTR("EE Reset Conf"));
      memcpy_P(&Config, &DefaultConfig, sizeof(Config));
      SaveConfig();
   ...
   */ 
    ...
    Serial.println("Loaded sensors: ");
    for (byte i = 0; i < ITPLUS_MAX_SENSORS; i++)
    { 
      Serial.print(Config.ITPlusID[i]); Serial.write(" - "); Serial.print(i);  Serial.println();     
